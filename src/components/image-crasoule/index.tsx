...
import { carouselImageData } from "@/data";
  ...
  return (
    <div className="px-3 md:px-4 lg:px-10">
      ...
      <h3 className="p-2 font-inter text-2xl font-semibold text-black">
        {carouselImageData[ActiveIndex].heading}
      </h3>
      <h2 className="px-2 py-3 text-xl">
        {carouselImageData[ActiveIndex].descr}
      </h2>
    </div>
  );
};
...
const Dots = ({ ActiveIndex }: { ActiveIndex: number }) => {
  return (
    <div className="flex items-center gap-3 p-2">
      <span
        className={`size-3 rounded-full ${ActiveIndex === 0 ? "bg-[#00172f]" : "bg-slate-200"}`}
      />
      <span
        className={`size-3 rounded-full ${ActiveIndex === 1 ? "bg-[#00172f]" : "bg-slate-200"}`}
      />
      <span
        className={`size-3 rounded-full ${ActiveIndex === 2 ? "bg-[#00172f]" : "bg-slate-200"}`}
      />
      <span
        className={`size-3 rounded-full ${ActiveIndex === 3 ? "bg-[#00172f]" : "bg-slate-200"}`}
      />
    </div>
  );
};
